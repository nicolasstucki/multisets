...
class MapBag[A, G <: BagBucket[A, G]](multiplicityMap: mutable.Map[A, G])(implicit protected val m: BagBuckets[A, G]) extends mutable.Bag[A, G] {
  ...
  def groupIterator: Iterator[G] = multiplicityMap.valuesIterator
...
object MapBag {
  ...
  def apply[A, G <: BagBucket[A, G]](implicit m: BagBuckets[A, G]): mutable.Bag[A, G] = new mutable.MapBag[A, G](mutable.Map.empty[A, G])(m)

  def apply[A, G <: BagBucket[A, G]](elemCount: (A, Int))(implicit m: BagBuckets[A, G]): mutable.Bag[A, G] = new mutable.MapBag[A, G](mutable.Map[A, G](elemCount._1 -> m(elemCount)))

  def apply[A, G <: BagBucket[A, G]](elem1: (A, Int), elem2: (A, Int), elems: (A, Int)*)(implicit m: BagBuckets[A, G]): mutable.Bag[A, G] = {
  ...
  def apply[A, G <: BagBucket[A, G]](elems: scala.collection.Iterable[A])(implicit m: BagBuckets[A, G]): mutable.Bag[A, G] = new mutable.MapBag[A, G](mutable.Map() ++ (elems map (elem => elem -> m(elem -> elems.count(elem == _)))))
}