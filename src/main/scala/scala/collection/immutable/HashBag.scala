...
class HashBag[A] private[collection](contents: mutable.HashTable.Contents[A, mutable.DefaultEntry[A, immutable.BagBucket[A]]])(implicit protected val bagConfiguration: immutable.HashedBagConfiguration[A])
  ...
  with Serializable {
    ...
    // TODO: change mutable.HashTable for immutable hash trie
    //    class BagHashTrie extends HashMap[A, immutable.BagBucket[A]] {
    //        override protected def elemHashCode(key: A) = bagConfiguration.equivClass.hash(key)
    //    }
